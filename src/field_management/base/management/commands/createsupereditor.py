...
from base.choices import SUPEREDITOR
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help='Specifies the password for the editor.',
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if username:
            ...
            if options['verbosity'] >= 1:
                ...
                user_create.update(is_superuser=False)
                user_create.first().groups.add(Group.objects.get(name='super_editor'))

                # Option supereditor
                op = Option.objects.filter(user_id=user_create.first().id)
                op.update(option=SUPEREDITOR)

                self.stdout.write("Supereditor created successfully.")
